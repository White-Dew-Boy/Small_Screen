 ...
 *
 * Tapping a file row selects it (enables Delete) and shows its name + size.
 * Audio playback is not handled here: it has its own page (gui/ui_player.c),
 * opened from the SD card page. The only trace of the player left in this
 * file is the Delete guard: unlinking a file the player still has open would
 * let its FATFS handle outlive the clusters. */
...
static void do_delete_selected(void)
{
     ...
     * outlive the clusters. This is a safety guard, not a UI feature — the
     * track is stopped from the Audio page. */
...
lv_obj_t *ui_files_create(void)
{
    ...
    /* Bottom row: Delete (disabled until a file is selected) / Back. */
    s_delete_btn = lv_btn_create(s_scr);
    lv_obj_set_size(s_delete_btn, 120, 34);
    ...
    lv_obj_set_size(back_btn, 120, 34);
